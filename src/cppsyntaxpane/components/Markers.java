...
public class Markers {
  ...
  public static void removeMarkers (JTextComponent component, SimpleMarker marker) {
    ...
    for (Highlighter.Highlight hilite1 : hilites) {
      if (hilite1.getPainter() instanceof SimpleMarker) {
        SimpleMarker hMarker = (SimpleMarker) hilite1.getPainter();
   ...
   */
  static void markToken (JTextComponent pane, Token token, SimpleMarker marker) {
   ...
   */
  private static void markText (JTextComponent pane, int start, int end, SimpleMarker marker) {
    try {
